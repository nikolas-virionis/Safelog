...
axios
    .post(`/machine/lista-dependentes/${cargoPessoa}`, {
        id
    })
...
maquinas.addEventListener("change", e => {
    ...
    if (maquinas.value) {
        ...
        axios
            .post("/machine/lista-componentes", {id: Number(maquinas.value)})
            .then(({data: {status, msg}}) => {
                if (status === "ok") {
                    option = document.createElement("option");
        ...
        axios
            ...
            .then(({data: {status, msg, continuar}}) => {
                if (status === "ok") {
                    ...
                    setTimeout(
                        () => (window.location.href = "safedesk-calls"),
                        1500
                    );

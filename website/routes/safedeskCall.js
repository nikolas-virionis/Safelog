...
const {abrirChamado, getTipo} = require("../util/safedesk-call/opening");
const {
    verificarAcesso,
    usuariosComAcesso
} = require("../util/safedesk-call/access");
const {getStatsChamado} = require("../util/analytics/data");
...
const {dadosUsuario} = require("../util/user/data");
const {
    maquinasDependentes
...
router.post("/lista", async (req, res) => {
    ...
    dadosUsuario(idUsuario).then(({msg: {cargo}}) => {
        maquinasDependentes(cargo, idUsuario).then(
            async ({status, msg: maquinas}) => {
                if (status == "ok") {
                    let idCategorias = [];
                    for (let {pk_maquina} of maquinas) {
                        ...
                        await sequelize
                            ...
                            .catch(async err => {
                                return Promise.resolve(
                                    await sequelizeAzure.query(sqlCategorias, {
                                        type: sequelizeAzure.QueryTypes.SELECT
                                    })
                                );
                            })
                            .then(categorias => {
                                for (let {id_categoria_medicao} of categorias) {
                                    idCategorias.push(id_categoria_medicao);
                                }
                            });
                    }
                    ...
                    await sequelize
                        ...
                        .catch(async err => {
                            return Promise.resolve(
                                await sequelizeAzure.query(sqlChamados, {
                                    type: sequelizeAzure.QueryTypes.SELECT
                                })
                            );
                        })

...
const {
    ...
    checarEmStaff,
    checarEmUsuario
} = require("../util/register/invite");
const {
    updateDadosUsuario,
    criarFormasContato
} = require("../util/register/updateData");
const {generateToken} = require("../util/token-user/token");
const {mandarEmail} = require("../util/email/email");
const {deleteUsuario} = require("../util/delete-user/delete");
const {
    redirecionamentoAcessos
} = require("../util/machine-access/accessChoice");
const {deleteAcesso} = require("../util/machine-access/deleteAccess");
...
router.post(
    ...
    async (req, res, next) => {
        ...
        sequelize
            ...
            .then(response => {
                res.redirect(`/profile-edit?update-pic=true&pic=${filename}`);
            })
            .catch(err => res.json({status: "erro", msg: err}));
    }
);
...
router.post("/machine-access", async (req, res) => {
